"""The shared body of EXECUTION and EXECUTION_LIGHT.

The two levels differ in exactly one thing: whether each workflow is driven by
the browser with per-frame video capture, or run server-side with a single
screenshot at the end. Everything else -- workflow selection, skip semantics,
resource monitoring, per-workflow logs, results.json, the HTML report -- is
identical.

They used to be two 300-line copies sharing 159 identical lines, and the cost
was not duplication for its own sake. Three fixes landed in `execution.py` and
were never carried across:

* the CPU/CUDA routing guard, whose absence let a mis-keyed config run every
  workflow on a runner configured for three;
* checking the skip list *before* `free_memory` and `add_log_listener`, whose
  absence leaked one log listener per skipped workflow, so every subsequent
  log line was recorded N+1 times;
* saving and clearing the browser console log per workflow.

One body, one `capture` flag. A fix now lands in both by construction.
"""

import json
import os
import subprocess
import threading
import time
from datetime import datetime, timezone
from pathlib import Path
from typing import List, Optional

from ...common.accel import is_cuda_run
from ...common.errors import (
    TestError,
    TestTimeoutError,
    WorkflowError,
    WorkflowExecutionError,
)
from ...common.resource_monitor import ResourceMonitor
from ..context import LevelContext, require_workflows, resolve_lane_id
from ..results import get_hardware_info, get_workflow_timeout


class ProgressSpinner:
    """Progress indicator for workflow execution."""

    def __init__(self, workflow_name: str, current: int, total: int):
        self.workflow_name = workflow_name
        self.current = current
        self.total = total
        self.start_time = time.time()
        self._stop = False
        self._thread: Optional[threading.Thread] = None

    def start(self) -> None:
        """Start the spinner animation in a background thread."""
        self._thread = threading.Thread(target=self._spin, daemon=True)
        self._thread.start()

    def _spin(self) -> None:
        """Print workflow start - no animation."""
        print(f"executing {self.workflow_name} [{self.current}/{self.total}]")
        while not self._stop:
            time.sleep(0.1)

    def stop(self, status: str, peak_vram_gb=None, peak_ram_gb=None) -> None:
        """Stop and print final status."""
        self._stop = True
        if self._thread:
            self._thread.join(timeout=0.3)
        mins, secs = divmod(int(time.time() - self.start_time), 60)
        metrics = []
        if peak_vram_gb is not None:
            metrics.append(f"Peak VRAM: {peak_vram_gb:.2f} GB")
        if peak_ram_gb is not None:
            metrics.append(f"Peak RAM: {peak_ram_gb:.2f} GB")
        metrics_str = f"  ({' | '.join(metrics)})" if metrics else ""
        print(f"[{mins:02d}:{secs:02d}] {self.workflow_name} "
              f"[{self.current}/{self.total}] - {status}{metrics_str}")


def _resolve_workflow_path(ctx: LevelContext, workflow_file: Path) -> Path:
    """Resolve workflow file path relative to node directory."""
    workflow_path = Path(workflow_file)
    if not workflow_path.is_absolute():
        workflow_path = ctx.node_dir / workflow_file
    return workflow_path


def _select_workflows(ctx: LevelContext, workflows, level_name: str):
    """(allowed, runner_type) or None when this runner must run nothing.

    An empty allowed list disables the skip filter, which is right for a pack
    that never configured routing at all. But when the OTHER accelerator's list
    IS configured, the pack has expressed routing intent and an empty list here
    means "nothing runs on this runner" -- as WorkflowConfig has always said.

    Falling through to run-everything here is how a typo'd key ('gpu' instead
    of 'cuda') executed all 59 workflows on a runner configured to run 3,
    presented as a plausible 48/59 result.
    """
    cpu_workflows = set(ctx.config.workflow.cpu or [])
    cuda_workflows = set(ctx.config.workflow.cuda or [])
    if is_cuda_run():
        allowed, other, runner_type = cuda_workflows, cpu_workflows, "CUDA"
    else:
        allowed, other, runner_type = cpu_workflows, cuda_workflows, "CPU"

    if not allowed and other:
        ctx.log(
            f"{runner_type} runner - the {runner_type.lower()} list is empty "
            f"while the other accelerator list is configured; running nothing. "
            f"Populate [test.workflows] {runner_type.lower()} = [...] to run "
            f"workflows on this runner.")
        return None

    total = len(workflows)
    if allowed:
        names = sorted(w.stem for w in workflows if w in allowed)
        skipped = total - len(names)
        ctx.log(f"{runner_type} runner - will execute {len(names)} workflow(s): "
                f"[{', '.join(names)}]")
        if skipped:
            ctx.log(f"  ({skipped} workflow(s) not in the "
                    f"{runner_type.lower()} list; recorded as skipped)")
    else:
        ctx.log(f"{runner_type} runner - no workflows configured for this runner type")
        ctx.log(f"Running {total} workflow(s) ({'with videos' if level_name == 'execution' else '1 screenshot each'})...")
    return allowed, runner_type


def _node_commit_hash(ctx: LevelContext) -> Optional[str]:
    """SHA of the pack under test, or None.

    Only read when `.git` sits directly in node_dir -- otherwise git walks up
    to a parent repo (ComfyUI's own clone) and returns the wrong hash.
    """
    if not (ctx.node_dir / ".git").exists():
        return None
    try:
        # Docker bind mounts arrive with foreign ownership; git refuses those.
        subprocess.run(
            ["git", "config", "--global", "--add", "safe.directory", str(ctx.node_dir)],
            capture_output=True, timeout=5,
        )
        r = subprocess.run(["git", "rev-parse", "HEAD"], cwd=ctx.node_dir,
                           capture_output=True, text=True, timeout=5)
        if r.returncode == 0:
            return r.stdout.strip()
        ctx.log(f"  commit_hash: git rev-parse failed ({r.stderr.strip()})")
    except Exception as e:
        ctx.log(f"  commit_hash: could not read ({e!r})")
    return None


def run_workflows(ctx: LevelContext, *, capture: bool, level_name: str) -> LevelContext:
    """Execute the configured workflows and write results.json.

    Args:
        ctx: Level context (must have server, api set).
        capture: True to record per-frame video via the browser (EXECUTION);
            False to run server-side and take one screenshot (EXECUTION_LIGHT).
            The capture loop pegs a browser process at 100% CPU, which kills
            the Playwright IPC pipe on weak runners -- hence the light path.
        level_name: For messages and the require_workflows error.

    Raises:
        WorkflowExecutionError: If any workflow fails.
    """
    require_workflows(ctx, level_name)

    if ctx.server is None:
        from ...comfyui.server import ComfyUIServer
        ctx.log("Starting ComfyUI server...")
        ctx.server = ComfyUIServer(ctx.paths, ctx.platform, log_callback=ctx.log)
        ctx.server.start()
        from ...comfyui.api import ComfyUIAPI
        ctx.api = ComfyUIAPI(ctx.server.base_url, log_callback=ctx.log)

    workflows = ctx.config.workflow.workflows
    if ctx.config.get_platform_config(ctx.platform_name).skip_workflow:
        ctx.log("Skipped per platform config")
        return ctx

    if ctx.workflow_filter:
        workflows = [w for w in workflows
                     if w.name == ctx.workflow_filter or w.stem == ctx.workflow_filter]
        if not workflows:
            raise TestError(f"Workflow not found: {ctx.workflow_filter}")
        ctx.log(f"Workflow filter: running only {workflows[0]}")

    selection = _select_workflows(ctx, workflows, level_name)
    if selection is None:
        return ctx
    allowed_workflows, runner_type = selection
    total_workflows = len(workflows)

    current_workflow_log: List[str] = []

    def capture_log(msg):
        """Append to the per-workflow log (server log listener)."""
        current_workflow_log.append(msg)

    def capture_and_print(msg):
        """Append to the per-workflow log AND the session log."""
        current_workflow_log.append(msg)
        ctx.log(msg)

    from ...reporting.screenshot import (
        ScreenshotError,
        WorkflowScreenshot,
        check_dependencies,
        ensure_dependencies,
    )

    python_path = ctx.paths.python if ctx.paths else None
    if not ensure_dependencies(python_path=python_path, log_callback=ctx.log):
        raise TestError("Failed to install screenshot dependencies (playwright required)")
    check_dependencies()

    height = ctx.config.res
    width = int(height * 16 / 9)
    # EXECUTION routes browser chatter into the session log too; the light path
    # keeps the browser idle, so its output is not interesting per-workflow.
    ws = WorkflowScreenshot(ctx.server.base_url, width=width, height=height,
                            log_callback=capture_and_print if capture else ctx.log)
    ws.start()

    runner = None
    if not capture:
        # Python-side WebSocket polling; the browser stays idle throughout.
        from ...comfyui.workflow import WorkflowRunner
        runner = WorkflowRunner(ctx.api, log_callback=capture_log)

    screenshots_dir = ctx.output_base / "screenshots"
    screenshots_dir.mkdir(parents=True, exist_ok=True)
    videos_dir = ctx.output_base / "videos"
    if capture:
        videos_dir.mkdir(parents=True, exist_ok=True)

    results = []
    logs_dir = ctx.output_base / "logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    hardware = get_hardware_info()

    try:
        all_errors = []

        for idx, workflow_file in enumerate(workflows, 1):
            # Checked FIRST, before free_memory and before registering the log
            # listener: doing it the other way round hit the server's /free
            # endpoint once per skip (56 pointless calls on a cuda run) and
            # leaked one log listener each time, so every later log line was
            # recorded once per skip. Silent on purpose -- the executed set is
            # announced up front and skips appear in results.json.
            if allowed_workflows and workflow_file not in allowed_workflows:
                results.append({
                    "name": workflow_file.stem,
                    "status": "skipped",
                    "duration_seconds": 0,
                    "error": f"Not configured for {runner_type} runner",
                    "hardware": None,
                })
                continue

            ctx.api.free_memory(unload_models=True)
            current_workflow_log.clear()
            ctx.server.add_log_listener(capture_log)
            start_time = time.time()
            status = "pass"
            error_msg = None

            spinner = ProgressSpinner(workflow_file.name, idx, total_workflows)
            spinner.start()

            server_pid = getattr(ctx.server, 'pid', None)
            resource_monitor = ResourceMonitor(monitor_cuda=is_cuda_run(), server_pid=server_pid)
            resource_monitor.start()

            try:
                workflow_path = _resolve_workflow_path(ctx, workflow_file)
                final_screenshot_path = screenshots_dir / f"{workflow_file.stem}_executed.png"
                if capture:
                    ws._last_capture_start = None  # reset; set inside on success
                    ws._last_media_span = None     # video/scrubber duration
                    frames = ws.capture_execution_frames(
                        workflow_path,
                        output_dir=videos_dir / workflow_file.stem,
                        log_lines=current_workflow_log,
                        webp_quality=60,
                        final_screenshot_path=final_screenshot_path,
                        final_screenshot_delay_ms=5000,
                        timeout=get_workflow_timeout(ctx.config.workflow.timeout),
                    )
                    capture_log(f"    Captured {len(frames)} video frames")
                else:
                    runner.run_workflow(
                        workflow_path,
                        timeout=get_workflow_timeout(ctx.config.workflow.timeout),
                    )
                    try:
                        ws.capture(workflow_path, output_path=final_screenshot_path)
                        capture_log(f"    Saved screenshot: {final_screenshot_path.name}")
                    except ScreenshotError as e:
                        capture_log(f"    Screenshot failed (non-fatal): {e}")
            except (WorkflowError, TestTimeoutError, ScreenshotError) as e:
                status = "fail"
                error_msg = str(e)
                capture_log("    Status: FAILED")
                capture_log(f"    Error: {e.message}")
                if hasattr(e, 'details') and e.details:
                    capture_log(f"    Details: {e.details}")
                all_errors.append((workflow_file.name, str(e)))
            except Exception as e:
                status = "fail"
                error_msg = str(e)
                capture_log("    Status: FAILED (unexpected error)")
                capture_log(f"    Error: {e}")
                all_errors.append((workflow_file.name, str(e)))
            finally:
                duration = time.time() - start_time
                resource_metrics = resource_monitor.stop()
                peak_vram = resource_metrics.get("vram", {}).get("peak")
                peak_ram = resource_metrics.get("ram", {}).get("peak")
                spinner.stop("PASS" if status == "pass" else "FAIL",
                             peak_vram_gb=peak_vram, peak_ram_gb=peak_ram)
                ctx.server.remove_log_listener(capture_log)

                if resource_metrics.get("timeline"):
                    _write_resource_csv(
                        logs_dir / f"{workflow_file.stem}_resources.csv",
                        resource_metrics, resource_monitor, ws, capture)
                    resource_metrics.pop("timeline", None)

                results.append({
                    "name": workflow_file.stem,
                    "status": status,
                    "duration_seconds": round(duration, 1),
                    "error": error_msg,
                    "hardware": hardware,
                    "resources": resource_metrics,
                })

                (logs_dir / f"{workflow_file.stem}.log").write_text(
                    "\n".join(current_workflow_log), encoding="utf-8")
                # Browser console output only means something when the browser
                # actually drove the workflow.
                if capture:
                    ws.save_console_logs(logs_dir / f"{workflow_file.stem}_console.log")
                    ws.clear_console_logs()

                # Unload after each workflow so limited-VRAM GPUs do not OOM.
                try:
                    ctx.api.free_memory(unload_models=True)
                except Exception:
                    pass
    finally:
        ws.stop()

    passed_count = sum(1 for r in results if r["status"] == "pass")
    failed_count = sum(1 for r in results if r["status"] == "fail")

    from ...common.config import build_provenance
    results_data = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "lane": resolve_lane_id(ctx),
        "hardware": hardware,
        "comfyui_version": ctx.comfyui_version,
        # SHA of the PACK under test (kept under the legacy name the dashboard
        # reads); ComfyUI's own SHA is provenance.comfyui_commit.
        "commit_hash": _node_commit_hash(ctx),
        "comfyui_commit": ctx.comfyui_commit,
        "provenance": build_provenance(
            ctx.config, install_mode="attach" if ctx.server_url else "fresh"),
        # GHA run that produced this result. The dashboard's Goto-mode reads it
        # to deep-link cells back to the run.
        "run_url": os.environ.get("COMFY_TEST_RUN_URL") or None,
        "success": all(r["status"] == "pass" for r in results if r["status"] != "skipped"),
        "summary": {
            "total": len(results),
            "passed": passed_count,
            "failed": failed_count,
        },
        "workflows": results,
    }
    results_file = ctx.output_base / "results.json"
    results_file.write_text(json.dumps(results_data, indent=2), encoding='utf-8')
    ctx.log(f"Results saved to {results_file}")

    if ctx.paths and ctx.paths.comfyui_dir:
        from ..model_tracker import build_models_report, save_models_report
        models_dir = ctx.paths.comfyui_dir / "models"
        if models_dir.exists():
            report = build_models_report(models_dir)
            if report["folders"]:
                report_path = save_models_report(report, ctx.output_base)
                ctx.log(f"Model report: {report['summary']['total_files']} files, "
                        f"{report['summary']['total_size_human']} -> {report_path}")

    from ...reporting.html_report import generate_html_report
    html_file = generate_html_report(ctx.output_base, ctx.node_dir.name)
    ctx.log(f"Saved: {html_file}")

    if all_errors:
        raise WorkflowExecutionError(
            f"Workflow execution failed ({len(all_errors)} error(s))",
            [f"{name}: {err}" for name, err in all_errors],
        )

    return ctx


def _write_resource_csv(csv_path: Path, resource_metrics, resource_monitor,
                        ws, capture: bool) -> None:
    """Write the RAM/VRAM timeline, aligned to the video clock when there is one.

    The monitor starts before the browser navigates to ComfyUI, so its clock
    leads the video by the navigation time. On the capture path we shift by
    that offset and drop samples outside the recorded span, so the graph and
    the video share one clock. With no video there is nothing to align to.
    """
    total_ram = resource_metrics.get("total_ram_gb", 16)
    offset, media_span = 0.0, None
    if capture:
        cap_start = getattr(ws, "_last_capture_start", None)
        media_span = getattr(ws, "_last_media_span", None)
        if cap_start:
            offset = cap_start - resource_monitor._start_time
    with open(csv_path, 'w', encoding='utf-8') as f:
        f.write(f"# total_ram_gb={total_ram}\n")
        f.write("t,ram_gb,vram_gb\n")
        for sample in resource_metrics["timeline"]:
            t = round(sample['t'] - offset, 1)
            if t < 0:
                continue  # sampled before the video's first frame
            if media_span is not None and t > media_span + 0.5:
                continue  # sampled after the video ended
            vram_val = sample['vram'] if sample['vram'] is not None else ''
            f.write(f"{t},{sample['ram']},{vram_val}\n")
